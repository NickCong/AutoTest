...
export class StepComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    //   this.step=this.caseM.steps.find(f=>f.order==this.setId);
    //   this.Selname=SelectedItem.find(t=>t.label==this.step.type).value;
    //   this.Selname2=SelectedItem2.find(t=>t.label==this.step.textTag).value;
    //   this.Selname3=Operations.find(t=>t.label==this.step.action).value;
    //    }
  }
  addnew(): void {
    if(this.StepID==0){
